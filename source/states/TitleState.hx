...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		switch(easterEgg.toUpperCase())
		{
			...
			#if TITLE_SCREEN_EASTER_EGG
			case 'LEMO':
				gfDance.frames = Paths.getSparrowAtlas('GFLemo');
				gfDance.animation.addByPrefix('danceLeft', 'GFLemo', 24);
				gfDance.animation.addByPrefix('danceRight', 'GFLemo', 24);
			case 'SHADOW':
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 2:
					#if EVENTUAL_WATERMARKS
					createCoolText(['Eventual Engine by'], 40);
					#else
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			if (playJingle) //Ignore deez
			{
				...
				{
					case 'LEMO':
						sound = FlxG.sound.play(Paths.sound('JingleLemo'));
					case 'RIVER':
