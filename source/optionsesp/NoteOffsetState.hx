...
class NoteOffsetState extends MusicBeatState
{
	...
	{
		#if DISCORD_ALLOWED
		DiscordClient.changePresence("Menú de Delay/Offset del combo", null);
		#end
		...
		// Note delay stuff
		beatText = new Alphabet(0, 0, 'Latido!', true);
	...
	override public function update(elapsed:Float)
	{
		...
		if(controls.BACK)
		{
			...
			MusicBeatState.switchState(new optionsesp.OptionsState());
	...
	function reloadTexts()
	{
		for (i in 0...dumbTexts.length)
		{
			switch(i)
			{
				case 0: dumbTexts.members[i].text = 'Offset del Rating:';
	...
	function updateMode()
	{
		...
		var str2:String;
		if(onComboMenu)
			str = 'Offset del Combo';
		else
			str = 'Delay de las Notas/Latidos';

		if(!controls.controllerMode)
			str2 = '(Presiona Aceptar para Cambiar)';
		else
